 ...
 * when a tenant context is active (aligned with Commission/Payout/Activity services).
 */
class PartnerService
{
    ...
    public function createPartner(CreatePartnerDTO $dto): Partner
    {
        ...
        if ($dto->parentPartnerId) {
            $this->assertParentAccessible($dto->parentPartnerId, $tenantId);
        }
    ...
    /**
     * Parent may be same-tenant partner or a platform partner (tenant_id NULL).
     */
    private function assertParentAccessible(string $parentPartnerId, ?string $tenantId): void
    {
    ...
    /**
     * Multi-tier path: ancestor ids separated by "/", ending with immediate parent id.
     */
