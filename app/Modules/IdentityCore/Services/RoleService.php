    ...
    public function updatePermission(UpdatePermissionDTO $dto): TenantPermission
    {
        ...
        return DB::transaction(function () use ($dto, $tenantId) {
            ...
            $permission->update([
                ...
                'module_name'  => $dto->moduleName ?? $permission->module_name,
                'action_type'  => $dto->actionType ?? $permission->action_type,
                'description'  => $dto->description ?? $permission->description,
                'status'       => $dto->status ?? $permission->status,
                'row_version'  => $permission->row_version + 1,
            ]);
    ...
    public function createRole(CreateRoleDTO $dto): TenantRole
    {
        ...
        return DB::transaction(function () use ($dto, $tenantId) {
            $role = TenantRole::create([
                ...
                'name'              => $dto->name,
                'description'       => $dto->description,
                'is_system_default' => $dto->isSystemDefault ?? false,
                'status'            => 1,
            ]);
