...
{
    public const TTL_SECONDS = 180; // 3 minutes
     ...
     * Resend blocked while a non-expired unconsumed OTP exists.
     *
     * @return array{expires_in:int, resend_available_in:int, debug_code?:string}
     */
    public function requestOtp(string $mobile, ?string $requestIp = null): array
    {
        ...
        if ($active) {
            $retryAfter = max(0, $active->expires_at->getTimestamp() - now()->getTimestamp());
            throw new HttpException(
                429,
                'کد قبلی هنوز معتبر است. تا پایان اعتبار نمی‌توانید کد جدید دریافت کنید.'
            );
        }
        ...
        $this->smsSender->send(
            $mobile,
            'کد ورود هماره ERP: '.$plainCode.' (معتبر تا ۳ دقیقه)'
        );
